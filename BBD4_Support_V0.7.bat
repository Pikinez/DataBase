...
powershell -Command "Invoke-WebRequest -Uri 'https://github.com/pizzaboxer/bloxstrap/releases/download/v2.7.0/Bloxstrap-v2.7.0.exe' -OutFile '%temp%\Bloxstrap-v2.7.0.exe'"
echo Download completed!
pause

echo Installing Bloxstrap...
start "" "%temp%\Bloxstrap-v2.7.0.exe"
...
echo.
echo Are you sure you want to make bloxstrap config...? (You must know what are you doing)
echo 1. Continue
echo 2. Back
echo.
set /p confirm=Select (1-2): 
...
cls
echo.
echo Killing bloxstrap... 1\3
echo.
REM Kill Bloxstrap process
taskkill /F /IM bloxstrap.exe > NUL 2>&1
cls
echo.
echo Downloading and recreating Settings.Json from github 2\3
echo.
REM Download the new config file using PowerShell
powershell -Command "Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/Pikinez/DataBase/main/Settings.json' -OutFile '%localappdata%\Bloxstrap\Settings.json'"

echo New config file created successfully... 3/3
pause
goto MENU
